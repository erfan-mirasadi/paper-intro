...
export default function CavePillars(props: any) {
  ...
  useEffect(() => {
    let isMounted = true;
    const url = "/assets/cave/pillars-opt.glb";

    if (gltfCache.has(url)) {
      Promise.resolve().then(() => {
        if (isMounted) setModelData(gltfCache.get(url));
      });
      return;
    }
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        gltf.scene.traverse((obj: any) => {
          ...
          if (obj.material) {
            // Use MeshStandardMaterial with envMap applied manually (not via global Environment)
        ...
        // if material names are somehow different, we just grab them sequentially
        // or by known node names
        const data = { nodes, materials };
        gltfCache.set(url, data);
        if (isMounted) setModelData(data);
      },
      undefined,
      (err) => {
        console.error(`❌ Error loading pillars model:`, err);
      },
    );
