...
export default function SweepRevealWrapper({
  ...
  }, [onRevealStart, dynamicEpicenter]);
      ...
      if ((child as THREE.Mesh).isMesh) {
        ...
        if (mesh.material) {
          ...
          mat.onBeforeCompile = (shader, renderer) => {
            ...
            // Convert the mode prop to a float uniform for the shader
            const isOverlayFloat = mode === "overlay" ? 1.0 : 0.0;

            // Register uniforms
            shader.uniforms.uTime = { value: 0 };
            shader.uniforms.uEpicenter = {
              value: new THREE.Vector3(...epicenter),
            };
            shader.uniforms.uRadius = { value: currentRadius.current };
            shader.uniforms.uTrailLength = { value: trailLength };
            shader.uniforms.uGlowColor = { value: new THREE.Color(beamColor) };
            shader.uniforms.uDarkness = { value: darkness };
  ...
  }, [children, epicenter, beamColor, mode, darkness, trailLength]); // Added darkness and trailLength to dependencies

  useFrame((state, delta) => {
    if (epicenterUpdateFrames.current > 0) {
      epicenterUpdateFrames.current--;
      if (epicenterUpdateFrames.current === 0) {
        const startPos = new THREE.Vector3(state.camera.position.x, epicenter[1], state.camera.position.z);
        shadersRef.current.forEach((shader) => {
          if (shader.uniforms && shader.uniforms.uEpicenter) {
